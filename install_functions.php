...
/**
 * Add all system modules not already defined in $modules.
 *
 * @param int $count how many of the new modules to add to the data set
 * @param array $modules exiting modules configuration
 * @return array
 */
function generate_full_module_list($count, $modules = array())
{
    global $moduleList;
    foreach ($moduleList as $aModule) {
        if (!isset($modules[$aModule])) {
            $modules[$aModule] = $count;
        }
    }
